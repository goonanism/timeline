...
class eventModel(timeline.model.Model):
	def __init__(self, name = 'Event', table = 'event', habtm = 'Tag', join_table = 'event_tag'):
		self.name = name
		self.table = table
		self.habtm = habtm
...
@app.route("/events/")
def events():
	data = {'Event' : {'id' : 1, 'name' : 'Karl Max'}}
	event_model.save(data)
	return 'event'
